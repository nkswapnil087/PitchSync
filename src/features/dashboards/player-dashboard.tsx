...
export function PlayerDashboard() {
  ...
  const playerId = session?.role === "player" ? session.personId : null;
  return (
    <>
      ...
      <section className="grid grid-cols-1 gap-3 sm:grid-cols-3"><Button asChild><Link href={playerId ? `/players/${playerId}` : "/player/dashboard"}><UserRound />Player profile</Link></Button><Button asChild variant="outline"><Link href={playerId ? `/performance/players/${playerId}` : "/player/dashboard"}><BarChart3 />Career & performance</Link></Button><Button asChild variant="outline"><Link href="/matches"><Medal />Matches</Link></Button></section>
