...
export function IntegrityCaseRegistry() {
  const { searchParams, setFilter, state } = useRegistryFilters("No integrity case records found.");

  return (
    <>
      <ConfidentialityNotice />
      <PageHeader eyebrow="Integrity domain" title="Integrity cases" description="Review case progress and its complaint, player, investigator, rule, and evidence relationships." actions={<Button asChild variant="outline"><Link href="/integrity/cases/record">Open case details</Link></Button>} />
      <FilterBar>
      ...
      <RegistryTable columns={columns} state={state} emptyTitle="No cases found" emptyDescription="No integrity cases match the current filters." />
